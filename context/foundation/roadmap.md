...
| S-12 | exercise-statistics        | view per-exercise history, estimated 1RM, and volume/tonnage              | S-06          | FR-024, FR-025, FR-026                            | done     |
...
### S-21: Optional RPE logging per round

- **Outcome:** client can optionally record RPE (Rate of Perceived Exertion, typically 1–10) for each exercise round when logging a session; trainer and client session readouts show RPE when present without requiring it for every set
- **Change ID:** optional-rpe-logging
- **Linear:** ZAW-53
- **PRD refs:** FR-015, FR-017 (extends per-round set logging); dedicated RPE FR not yet in PRD — add at `/10x-plan` if scope needs contract lock-in
- **Prerequisites:** S-06 for guided workout set logging; S-13 for edit-window upsert/delete semantics on `set_logs`
- **Parallel with:** S-12, S-18 once S-06 logging UX is stable
- **Blockers:** —
