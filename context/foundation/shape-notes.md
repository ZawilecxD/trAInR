...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction — trainer's daily work scattered across too many tools"
    - topic: "competitive insight"
      decision: "existing tools too complex and expensive for independent trainers"
    - topic: "persona scope"
    ...
    - topic: "MVP flow"
      decision: "9-step flow: signup → exercises → template → invite → client register → assign → calendar → log → dashboard"
    - topic: "timeline"
      decision: "3 weeks after-hours"
    - topic: "guardrails"
      decision: "client data privacy — trainer sees only their own clients"
  frs_drafted: 15
...
2. Trainer creates exercises in their exercise library
3. Trainer builds a plan template from those exercises (phases, sets/reps/load)
4. Trainer generates an invite link
5. Client registers via the invite link
6. Trainer assigns the plan template to the client (with a start date)
7. Client opens their calendar view, sees today's session
8. Client completes the session, logs actual metrics (weight, reps)
9. Trainer sees the logged session on their dashboard
...
- FR-001: Trainer can register with email+password or Google OAuth. Priority: must-have
  > Socrates: Counter-argument considered: "Supporting both email+password AND Google doubles the auth surface — ship one first." Resolution: acknowledged; dual auth adds implementation cost but lowers onboarding friction. Consider shipping email+password first, Google OAuth as fast-follow if needed.
...
- FR-009: Trainer can create a reusable plan template from their exercise library, organized into phases with prescribed sets/reps/load. Priority: must-have
  > Socrates: No counter-argument; it stands as written.
- FR-010: Trainer can edit plan templates. Priority: must-have. Duplicate: nice-to-have
...
- **When** they generate an invite link, the client registers through it, and the trainer assigns the plan template to the client with a start date
- **Then** the client sees their sessions in a calendar day view and can log actual metrics for each session, and the trainer sees the logged activity on their dashboard

#### Acceptance Criteria
- Invite link leads to a registration page; client is auto-assigned to the trainer on completion
- Assigned plan appears in the client's calendar starting from the specified start date
- Client can record weight/reps/time/distance per exercise in a session
...
2. **No subscription/billing system** — free tier only at launch. Monetization is a Phase 4 concern.
3. **No client goals and progress tracking** — no weight/measurement/check-in tracking. Phase 2 roadmap item.
4. **No advanced analytics or reports** — no volume trends, adherence heatmaps, or client comparison. Phase 5 roadmap item.
5. **No notifications** — no email or push notifications. Phase 3 roadmap item.
6. **No calendar integrations** — no Google Calendar sync, no iCal export. Phase 3 roadmap item.
...
9. **No multi-trainer organizations or gym-level accounts** — single-trainer model only. Phase 5 roadmap item.
10. **No AI-powered plan generation or exercise suggestions** — all plan creation is manual. Phase 5 roadmap item.
11. **No audit logging** — no compliance event log. Phase 5 roadmap item.
