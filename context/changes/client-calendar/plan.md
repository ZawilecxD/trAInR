...
- [ ] 3.4 Client sees "My Plan" nav, opens /client/plan, sees current month with status dots
- [ ] 3.5 Day selection shows view-only session list; no add/edit/delete anywhere
- [ ] 3.6 Month navigation refetches; error surfaces on failure
- [ ] 3.7 Empty month shows a single friendly message; grid still renders; empty days show no dot
- [ ] 3.8 Month view usable one-handed on phone-width viewport
