...
- [ ] 3.1 `npm run lint` and `npm run build` both pass on final branch state
- [ ] 3.2 API and UI tests for exercise flows pass in CI-equivalent local run

#### Manual

- [ ] 3.3 SQL ownership verification script confirms trainer A cannot read/update trainer B exercises
- [ ] 3.4 End-to-end trainer flow (`create -> filter -> edit -> archive`) passes on local reset
- [ ] 3.5 No regression in auth middleware behavior for `/trainer` protected routes
