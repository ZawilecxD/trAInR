...
import { startMySession } from "@/lib/workout-sessions/service";
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await startMySession(supabase, guard.userId, parsedId.data);
  if (!result.ok) {
    if (result.code === "already_started") {
      return jsonError("already_started", 409, { message: result.message });
    }
