...
import { markSessionComplete } from "@/lib/workout-sessions/service";

export const prerender = false;

const completeBodySchema = z.object({
  status: z.enum(["finished", "finished_partially", "cancelled"]),
});
...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    return jsonError("validation_error", 400, { message: "Invalid JSON body" });
  }
  ...
  const result = await markSessionComplete(supabase, guard.userId, parsedId.data, parsed.data.status);
  if (!result.ok) {
    if (result.code === "already_completed") {
      return jsonError("already_completed", 409, { message: result.message });
    }
