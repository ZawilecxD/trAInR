-- Fix TOCTOU races in update_workout_session_snapshot and delete_workout_session.
-- Both RPCs check status/started_at then destructively mutate the session. Without
-- a row-level lock, a concurrent client start (started_at set) can slip between
-- the guard and the mutation. Adding FOR UPDATE OF ws serializes access to the
-- session row within the transaction and closes the race.
...
-- ---------------------------------------------------------------------------
-- Add exercise count cap (max 50) to create_workout_session, matching schema.
